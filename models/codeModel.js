...
exports.create = ({ code, batch_id }) => {
  return db.prepare('INSERT INTO codes (code, batch_id) VALUES (?, ?)').run(code, batch_id);
};

exports.getAll = () => db.prepare('SELECT * FROM codes').all();

exports.getByBatch = (batchId) => db.prepare('SELECT * FROM codes WHERE batch_id = ?').all(batchId);

exports.getNextAvailable = () => db.prepare('SELECT * FROM codes WHERE used = 0 ORDER BY id ASC LIMIT 1').get();

exports.markUsed = (id) => db.prepare('UPDATE codes SET used = 1 WHERE id = ?').run(id);
